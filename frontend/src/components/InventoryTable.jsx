...
import { ShoppingBag, ShoppingCart, Eye, Edit2, Trash2, ArrowUp, ArrowDown, ArrowUpDown, FileSpreadsheet } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between px-2">
        <div className="space-y-1">
          <h3 className="text-2xl font-black text-slate-800 uppercase tracking-widest">Stock Inventory</h3>
          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Manage your products and levels</p>
        </div>
        <button
          onClick={onExport}
        ...
        <div className="flex flex-col gap-4 sm:hidden">
          {products.length === 0 ? (
            ...
            products.map((product) => {
              ...
              return (
                <div key={product._id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center border border-gray-100 overflow-hidden shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-slate-400 font-bold tracking-tighter uppercase">{product.category}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 py-3 border-y border-slate-50">
                    <div>
                      ...
                      <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-[10px] text-slate-400 font-bold">/ {product.minStock}</span>
                      </div>
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Price</p>
                      <p className="text-sm font-black text-blue-600 font-mono">Rs. {product.price.toLocaleString('en-PK')}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-3 pt-1">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => addToCart(product)} className="p-2.5 text-blue-600 bg-blue-50 rounded-xl" title="Add to Cart">
                        <ShoppingCart className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleView(product)} className="p-2.5 text-slate-400 bg-slate-50 rounded-xl" title="View">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button onClick={() => onEdit(product)} className="p-2.5 text-blue-600 bg-blue-50 rounded-xl" title="Edit">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => onDelete(product._id)} className="p-2.5 text-red-500 bg-red-50 rounded-xl" title="Delete">
                  ...
                  products.map((product) => {
                    ...
                    return (
                      <tr key={product._id} className="hover:bg-slate-50/80 transition-all duration-200 group">
                        ...
                        <td className="px-6 py-6 whitespace-nowrap">
                          <div className="text-sm font-black text-slate-900">{product.stock} items</div>
                        ...
                        <td className="px-6 py-6 whitespace-nowrap">
                          {product.expiryDate ? (
                            <div className="flex flex-col">
                              <span className={`text-[10px] font-black uppercase tracking-tight ${new Date(product.expiryDate) < new Date() ? 'text-red-600' :
